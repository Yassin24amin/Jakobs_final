  ...
  handler: async (ctx) => {
    ...
    for (const ingredient of all) {
      ...
      if (ingredient.expiryDate <= now) {
        ...
        // Create urgent reorder with expiry reason (inline instead of generic checkAndCreateReorder)
        ...
        if (!hasPending) {
          ...
          await ctx.db.insert("im_reorders", {
            ...
            status: "ordered",
            trigger: "expiry",
            reason: `Expired: ${expiredQty} ${ingredient.unit} expired on ${expiryDateStr} and was discarded. Stock is now 0. Auto-ordered.`,
        ...
        await ctx.db.insert("im_reorders", {
          ...
          status: "suggested",
          trigger: "expiry",
          reason: `Expiring in ${daysLeft} day${daysLeft !== 1 ? "s" : ""}: ${ingredient.currentStock} ${ingredient.unit} of ${ingredient.name} expires soon. Pre-order recommended.`,
          estimatedCost: smartQty * ingredient.costPerUnit,
          createdAt: now,
          updatedAt: now,
        });

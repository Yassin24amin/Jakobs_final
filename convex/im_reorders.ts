...
/** Dashboard summary: counts of each status (indexed queries, no full scan) */
...
export const checkAndCreateReorder = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Critical = ALWAYS act. If there's only a "suggested" or "approved" one, upgrade it.
    if (isCritical && hasPending) {
      const suggestedOrApproved = existing.find(
